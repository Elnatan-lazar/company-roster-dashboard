...
function CreateSoldierModal({
  platoons, onConfirm, onClose,
}: {
  platoons: PlatoonRow[];
  onConfirm: (form: { first_name: string; last_name: string; personal_id: string; email: string; role: string; primary_platoon_id: string; }) => void;
  onClose: () => void;
}) {
  ...
  const valid = form.first_name.trim() && form.last_name.trim() && form.personal_id.trim() && form.email.trim();
  ...
  return (
    <ModalShell title="צור חייל חדש" onClose={onClose}>
      <div className="px-6 py-4 space-y-3">
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">מחלקה</label>
          <select
        ...
        <p className="text-[11px] text-gray-400">החייל יוכל להתחבר עם האימייל שצוין לאחר שיאופס סיסמתו.</p>
      </div>
      <div className="px-6 pb-5 flex gap-3 justify-end">
        <button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50">ביטול</button>
        <button
